...
-- Helper to verify if the user has staff or owner administration privileges without RLS recursion
...
begin
  ...
  select role into v_role from public.profiles where id = p_user_id;
  return v_role in ('owner', 'staff');
end;
$$;

-- Helper to verify if the user has owner privileges without RLS recursion
...
-- Secure helper function to fetch a user's account_status without triggering recursive RLS
...
-- Secure helper function to fetch a user's technician_status without triggering recursive RLS
create or replace function public.get_technician_status(p_user_id uuid)
returns text
language plpgsql
security definer
stable
...
begin
  ...
  select technician_status into v_tech_status from public.profiles where id = p_user_id;
  return v_tech_status;
end;
$$;
...
create policy "Users update own profile or owner"
  ...
  with check (
    ...
    or (
      ...
      and account_status = public.get_account_status(auth.uid())
      and technician_status is not distinct from public.get_technician_status(auth.uid())
    )
  );
...
create policy "Select quote items"
  ...
  using (
    ...
    or exists (
      ...
      where q.id = quote_items.quote_id and q.customer_id = auth.uid()
    )
  );
